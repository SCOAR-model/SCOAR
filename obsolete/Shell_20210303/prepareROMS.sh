...
if [ $vROMS -eq 38 ]; then
echo "for ROMSv38, I update the initial time to match the bry/frc"
echo "previously it was set to 0 and worked, but not in v3.8"

## update time variables of the initial file with juland date  (in seconds)
	## added 06/16/2017
echo "update time variables of the initial file with juliand date (in seconds)"
...
echo $var_time " --> " $JD
  ...
  echo $itime > fort.13
  $Couple_Lib_exec_coupler_Dir/update_init_time.x || exit 8
fi
