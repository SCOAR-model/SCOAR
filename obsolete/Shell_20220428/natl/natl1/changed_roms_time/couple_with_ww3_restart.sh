...
mkdir -p  $ROMS_Forc_Dir/$YYYYi  || exit 8
        ...
        ln -fs $ROMS_Forc_Dir/forc_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc $Couple_Data_ROMS_Dir/ocean_frc.nc || exit 8
...
if [ $ROMS_Avg = yes ]; then
	mv $Couple_Data_ROMS_Dir/ocean_avg.nc $ROMS_Avg_Dir/avg_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc || exit 8
fi

# rst
        mv $Couple_Data_ROMS_Dir/ocean_rst.nc $ROMS_Rst_Dir/rst_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc || exit 8

# his
if [ $ROMS_His = yes ]; then
        mv $Couple_Data_ROMS_Dir/ocean_his.nc $ROMS_His_Dir/his_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc || exit 8
fi
# qck
if [ $ROMS_Qck = yes ]; then
        mv $Couple_Data_ROMS_Dir/ocean_qck.nc $ROMS_Qck_Dir/qck_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc || exit 8
	...
	$NCO/ncks -F -O -d ocean_time,2 $ROMS_Qck_Dir/qck_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc $ROMS_Qck_Dir/$YYYYi/qck_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc
	###
fi
...
if [ $wave_current = yes ];then
	...
	# for inital case, jusy duplicate NHour for NHourm
       		...
       		ncks -3 -O -v u_sur_eastward,v_sur_northward,lon_rho,lat_rho $ROMS_Qck_Dir/qck_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc fort.11
	else
        	#ncks -3 -O -v u_sur_eastward,v_sur_northward,lon_rho,lat_rho $ROMS_Qck_Dir/qck_Hour$NHourm\.nc fort.11
       		ncks -3 -O -v u_sur_eastward,v_sur_northward,lon_rho,lat_rho $ROMS_Qck_Dir/qck_$YYYYi-$MMi-$DDi\_$HHi\_Hour$NHourm\.nc fort.11
	fi
        #ncks -3 -O -v u_sur_eastward,v_sur_northward,lon_rho,lat_rho $ROMS_Qck_Dir/qck_Hour$NHour\.nc fort.12
        ncks -3 -O -v u_sur_eastward,v_sur_northward,lon_rho,lat_rho $ROMS_Qck_Dir/qck_$YYYYin-$MMin-$DDin\_$HHin\_Hour$NHour\.nc fort.12
        ncrcat -O fort.11 fort.12 fort.11;      rm fort.12
