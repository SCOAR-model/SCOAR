...
#############USER DEFINED#########################
# ideally should be read from main.sh or crocotools_param.m
...
map = Basemap(width=12000000,height=9000000,
            ...
            lat_1=lat_1,lat_2=lat_2,lat_0=lat_1,lon_0=lon_0)
...
#h=np.zeros((Mp,Lp))
h = griddata(points,values,(hgrd.lon_rho,hgrd.lat_rho),method=interp_method)
...
#h = creep.cslf(h, np.nan, -200., 3200.)
...
RoughMat = bathy_tools.RoughnessMatrix(h, hgrd.mask_rho)
print('Max Roughness value is: ', RoughMat.max())
h = pyroms_toolbox.shapiro_filter.shapiro2(h, 2,scheme=2,napp=2)
#h = pyroms_toolbox.shapiro_filter.shapiro2(h, 2,scheme=2,napp=2)


#manual edit bathy close to coast specific locations:
#for i in range(77,139):
#	for j in range(98,162):
#		if h[j,i] > 45.:
#			h[j,i] = 45.
#for i in range(139,188):
#	for j in range(133,162):
#		if h[j,i] > 45.:
#			h[j,i] = 45.
#for i in range(231,279):
...
#
#tmp_file='f2name'
#ncid = netCDF4.Dataset(tmp_file, "w")
#h_tmp1 = ncid.variables['h'][:]
#
#h_tmp=h_tmp1[:]
#
##manual edit bathy close to coast specific locations:
