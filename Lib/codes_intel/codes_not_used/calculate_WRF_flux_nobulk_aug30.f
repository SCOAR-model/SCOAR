...
! 26. LH: W/m2
! 27. HFX: W/m2
! 29. RAINC [mm/CF] !note input rains are already in rain-rate check WRF2ROMS.sh
! 30. RAINNC [mm/CF] !mm/24hr or mm/6hr for example
! 31. SFCEVP kg/m2
...
! input
      ...
      real, dimension(:,:), allocatable :: rainnc, rainc
...
!      read(12,*) nxu, nyu, nxnyu
 !	 print *, nxu, nyu, nxnyu
!

!      open(13,file='fort.13',form='formatted')
!      read(13,*) nxv, nyv, nxnyv
 !	 print *, nxv, nyv, nxnyv
...
! input
      ...
      allocate(rainnc(nx,ny),rainc(nx,ny),rain(nx,ny))
...
!       open(14,file='fort.13',form='formatted')
!       read(14,*) masku
!       open(15,file='fort.13',form='formatted')
!       read(15,*) maskv
...
! ignore rainsh (too small)
      do i=1,nx
      do j=1,ny
! mm/hr --> 1/10cm / 1/24day --> 24/10 cm/day
        rain(i,j)=(rainc(i,j)+rainnc(i,j))/CF*24/10 !mm/hr --> cm/day
      enddo
      enddo
