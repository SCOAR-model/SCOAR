...
! - double rotation of current vector components
      ...
      real*8,dimension(:,:,:,:,:), allocatable :: uROMSu, vROMSv
      ...
      integer start(5), count(5), stride(5), count3(5)
...
! ROMS eta-grid dimensions
      ...
      allocate(uROMSu(nxu,nyu,1,1,1))
      allocate(vROMS(nx,ny,1,1))
      allocate(vROMSv(nxv,nyv,1,1,1))
...
! interpolate from u- to rho-grid
      do j=1,ny
        do i=2,nx
         uROMS(i,j,1,1)=0.5 * (uROMSu(i-1,j,1,1,1)+uROMSu(i,j,1,1,1))
        enddo
      enddo
...
! interpolate from v- to rho-grid       
      do j=2,ny
        do i=1,nx
          vROMS(i,j,1,1)=0.5 * (vROMSv(i,j-1,1,1,1)+vROMSv(i,j,1,1,1))
        enddo
      enddo
...
!! special case...
          if (int(land(i,j,1)) .eq. 1 .and. uWRF(i,j,1)>500) then
             uWRF(i,j,1) = 0.
             vWRF(i,j,1) = 0.
          endif
        enddo
