! Add ROMS SST to wrflowinp from restart file with cpp option PERFECT_RESTART.
! Modified from sst_wrflowinp_nolake_initial.f
! Main modifications:
! - deal with extra dimension in PERFECT_RESTART rst.nc file.
        ...
        do 200 i=1,nx
         if (int(land(i,j,1)) .eq. 0) then !in roms 0 is land
           if (int(land_wrf(i,j,1)+lake_wrf(i,j,1)) .eq. 0) then !in wrf 0 is ocean
            sstROMS(i,j,1,1,1) = sstWRF(i,j,1) !ROMS is land but WRF is ocean
           else 
            sstROMS(i,j,1,1,1) = 0 !both are land 
           endif
         endif
  200    continue
!print *, sstROMS
