...
! note convetion is [x y z t]!! not following netcdf convention!
      status = nf_get_vars_double(ncid,varid,start,count,stride,vROMSv)
...
! u to rho grid
       ...
       do i=1,nx
        vROMS(i,1,1,1)=vROMSv(i,2,1,1)
        vROMS(i,ny,1,1)=vROMSv(i,ny-1,1,1)
        enddo
...
! ###########
      ...
      call exit
       ...
       if (status .ne. nf_noerr) then
          print *, n,' ',status
          print *, 'reading voce from avg.nc failed!!!'
          print *, 'stop'
           call exit
       endif
