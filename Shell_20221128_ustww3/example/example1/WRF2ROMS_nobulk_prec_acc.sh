...
# use accumulated precipitation (set prec_acc_dt in namelist.input!!!)
for VAR in U10 V10 UST GSW GLW SST LH HFX PREC_ACC_C PREC_ACC_NC COSALPHA SINALPHA; do
...
### >>> IMPORTANT <<< this is the time step of the parent domain!!!
## Feb 27 2020
#grep  "time_step "  $Couple_Lib_exec_WRF_Dir/$WRF_Namelist_input  | awk '{print $3}'  | sed 's/,//g' > fort.18
...
for VAR in U10 V10 UST GSW GLW SST LH HFX PREC_ACC_C PREC_ACC_NC COSALPHA SINALPHA utaw vtaw utwo vtwo
 ...
 ii=`expr $ii + 1 `
   ...
   $Couple_Lib_exec_coupler_Dir/calculate_WRF_flux_nobulk_prec_acc_tauoc.x || exit 8
	...
	# fort.54: swrad [W m-2]
	# fort.55: swflux 
	$Couple_Lib_exec_coupler_Dir/calculate_WRF_flux_nobulk_prec_acc.x || exit 8
